...
export const getContactUsFromAPI = async () => {
    ...
    await axios.get(`${process.env.REACT_APP_BACKEND_ROOT_ADDRESS}/api/contactUs`)
...
export const addContactUsToAPI = async (contactUs) => {
    try {
        const response = await axios.post(`${process.env.REACT_APP_BACKEND_ROOT_ADDRESS}/api/contactUs/add`, contactUs);
...
export const deleteContactUsFromAPI = async (id) => {
    try {
        await axios.get(`${process.env.REACT_APP_BACKEND_ROOT_ADDRESS}/api/contactUs/delete/${id}`);
    } catch (error) {
